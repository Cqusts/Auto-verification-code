...
| **智能字段识别** | 区分「短信验证码框」与「图片验证码框」，支持分格输入框、Shadow DOM、iframe；认不出时可手动指定并按站点记住 |
| **多种短信来源** | WebSocket 推送 / HTTP 轮询 / 剪贴板 / 手动粘贴 |
| **自动填写手机号** | 在设置里存自己的号码，遇到手机号输入框自动填入；验证码/密码/身份证等一律排除 |
...
**设置 → 常规 → 自动填写手机号**，填上你的号码。之后遇到「手机号 / 电话 / mobile / `type="tel"`」这类输入框会自动填入，配合短信验证码自动填写，登录基本不用动手。

留空则这个功能完全不生效。验证码框、密码、邮箱、搜索框、身份证、银行卡一律排除；默认不覆盖已有内容，手动清空后也不会被再填回去。

> 号码明文保存在 `chrome.storage.local`。这和验证码不同 —— 验证码只在内存里、关浏览器就清掉，而手机号要长期保留才有意义。不想留就把设置里那栏清空。
...
**页面上的验证码输入框没被识别到。** 点开扩展弹窗 →「**没识别到？手动指定输入框**」→ **关掉弹窗**，在页面上点一下那个输入框。页面上会弹出绿色的「已指定」确认。之后这个站点就一直记住了，刷新和重新打开都有效；弹窗里能看到已指定的选择器，也能一键清除。

取词模式在**所有 frame 里同时生效**，所以登录表单在 iframe 里（不少 SPA / 单点登录页是这样）也能点中。指定之后如果已经有一条未使用的验证码在等着，会立刻填进去，不用再发一遍。

自动识别覆盖了绝大多数写法（含旁边有「获取验证码」按钮但字段完全没有命名的情况），但总有站点什么线索都不给 —— 那就用手动指定，五秒钟的事。
...
**图片验证码报 `Failed to fetch` / 连不上识别服务。** 识别引擎设成了「自建 HTTP 接口」但 ddddocr 服务没在跑。要么双击 `start-ocr.cmd` 启动它，要么在设置里把引擎改回「本地离线」。设置里的 **测试接口** 按钮可以直接确认接口通不通。接口不可用时会自动退回内置引擎，所以不会整个用不了。
