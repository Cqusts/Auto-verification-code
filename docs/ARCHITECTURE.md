...
`验证码` 这个词的歧义**只在图片验证码还有可能时才成立**。字段附近找不到任何图片时，「图片验证码」这个读法就死了 —— 没有图的图片验证码框根本没法填，代码里本来也会丢弃它 —— 于是标签按排除法归到短信。这一条很关键：`<input placeholder="请输入验证码">` 没有 id、没有 name、旁边也没有按钮时，只靠歧义分 30 够不到 55 的门槛会被丢掉，而它的 placeholder 已经把自己是什么说得清清楚楚。
