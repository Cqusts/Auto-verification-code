...
| 组件 | 开机自动？ | 说明 |
| --- | --- | --- |
| 扩展本体 | 是 | 浏览器启动时自动加载，不用做任何事 |
| 图片验证码识别 | 是 | 属于扩展的一部分，不依赖任何外部服务 |
| 桥接服务（短信通路） | **装过自启才会** | 见下面那条命令 |

自启是**一次性安装**：在你自己的电脑上跑一次 `npm run autostart:install`，之后每次开机自动运行。**没跑过这条命令，开机后短信那条路就是断的** —— 图片验证码不受影响，照常工作。

### 三个前提，破坏任何一个都会失效

安装时会把**绝对路径**写进服务配置，所以：

1. **仓库目录不能移动或删除。** `bridge/server.mjs` 的路径被写死在服务配置里，移动之后服务会静默失败。需要移动就：`npm run autostart:uninstall` → 移动 → 在新位置 `npm run autostart:install`。
2. **Node 不能卸载或换路径。** 配置里记的是安装那一刻的 `node` 可执行文件路径。用 nvm 之类的工具切换了 Node 版本导致路径变化，就要重装一次自启。
3. **`extension/` 目录同样不能移动。** 开发者模式加载的扩展记的也是绝对路径，移动之后浏览器下次启动就找不到它了。

不确定现在还正不正常，跑一句 `npm run autostart` —— 它会真的去访问端口，直接告诉你在不在跑。
